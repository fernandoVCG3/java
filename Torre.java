...
public class Torre extends Pieza implements Movimientos {
    public Torre(String nombrePieza, String colorPieza, int pocision) {
    ...
    public void mostrarPosiblesMovimientos(ArrayList<Pieza> piezas) {
        System.out.println("Movimientos posibles de la Torre:");
        moverEnDireccion(piezas, 1, 0);
        moverEnDireccion(piezas, -1, 0);
        moverEnDireccion(piezas, 0, 1);
        moverEnDireccion(piezas, 0, -1);
    }
    ...
    public boolean jaque(ArrayList<Pieza> piezas) {
        for (Pieza pieza : piezas) {
            if (pieza instanceof Rey && !pieza.getColorPieza().equals(this.getColorPieza())) {
                if (this.getFila() == pieza.getFila() || this.getColumna() == pieza.getColumna()) {
                    return true;  // La torre puede atacar en línea recta
                }
            }
        }
        return false;
